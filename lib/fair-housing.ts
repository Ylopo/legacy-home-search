 ...
 *   2. checkFairHousing()  — verify generated content before Sanity save or social publish
 *
 * Redis keys:
 *   lhs:fh:{postId}   — FHCheckResult JSON, 90-day TTL
 */
...
All content must comply with the Fair Housing Act and Virginia fair housing law.
...
Intent is irrelevant. Protected classes: race, color, religion, national origin,
sex, disability, familial status, marital status, sexual orientation, gender identity,
source of income.
...
- Use "young professional" or "hip area" — these imply age/demographic preferences

ALWAYS:
- Describe the PROPERTY, its features, and objective amenities — never the ideal resident
- Use objective measurements for proximity: "0.4 miles from" or "8 minutes to," not
  "walking distance"
...
export type FHViolation = {
  excerpt: string       // the offending phrase or sentence
  reason: string        // which protected class / rule it triggers
...
export type FHCheckResult = {
  ...
  checkedAt: string
  reviewedAt?: string   // set when operator clicks "Mark as Reviewed"
}
...
PROTECTED CLASSES (federal + Virginia):
...
- "young professional," "young couples" as target audience
...
function fhKey(postId: string) { return `lhs:fh:${postId}` }
